...
def extract_results(df: pd.DataFrame, dataset_name: str, seed: int) -> List[RunRecord]:
    records: List[RunRecord] = []
    alg_col = None
    for cand in ['algorithm', 'Algorithm', 'algo', 'model']:
        if cand in df.columns:
            alg_col = cand
            break
    metric_cols = [c for c in df.columns if ('NDCG' in c or 'Precision' in c)]
    if alg_col is None or not metric_cols:
        return records
    for _, row in df.iterrows():
        algo = str(row[alg_col])
        for metric_col in metric_cols:
            val = row[metric_col]
            if pd.notnull(val):
                records.append(RunRecord(dataset_name, seed, algo, metric_col, float(val)))
    return records


def short_statistical_analysis(results: pd.DataFrame) -> pd.DataFrame:
    rows = []
    grouped = results.groupby(['dataset', 'algorithm', 'metric'])
    for key, grp in grouped:
        if not isinstance(key, tuple):
            key = (key,)
        dataset, algorithm, metric = key
...
def main() -> None:
    ...
    for dataset_name in dataset_names:
        for seed in seeds:
            ...
            candidates = [
                os.path.join(working_dir, f'{dataset_name}_seed_{seed}_results.csv'),
                os.path.join(working_dir, f'{dataset_name}_seed_{seed}.csv'),
            ...
            all_records.extend(extract_results(df, dataset_name, seed))
            run_metadata.append({
    ...
    if not results_df.empty:
        ...
        print('No results were collected. Check the OmniRec result file path emitted by run_omnirec.')
