...
def run_single_seed(seed, dataset_name, working_dir):
    set_random_state(seed)
    ds = load_dataset(dataset_name)
    ds = preprocess_dataset(dataset_name, ds)
    ds = RandomHoldout(validation_size=0.0, test_size=0.2).process(ds)
...
def normalize_results(df):
    ...
    if "metric" not in df.columns and any(c.lower() == "metric" for c in df.columns):
        df = df.rename(columns={next(c for c in df.columns if c.lower() == "metric"): "metric"})
    return df
...
def short_statistical_analysis(summary_df):
    ...
    for _, row in summary_df.iterrows():
        lines.append(
            f"{row['dataset']} / {row['algorithm']}: nDCG@10 mean={row.get('ndcg@10_mean', float('nan')):.4f}, "
            f"std={row.get('ndcg@10_std', float('nan')):.4f}, cv={row.get('ndcg@10_cv', float('nan')):.3f}"
        )
    return "\n".join(lines)
...
def main():
    ...
    for seed in seeds:
        for dataset_name in datasets:
            try:
                res_df = run_single_seed(seed, dataset_name, working_dir)
    ...
    metric_rows = []
    grouped = results_df.groupby(["dataset", "algorithm", "metric", "k"], dropna=False)
    for (dataset_name, algo, metric, k), grp in grouped:
    ...
    wide = summary_df.pivot_table(index=["dataset", "algorithm"], columns=["metric", "k"], values="mean")
    analysis_rows = []
    for (dataset_name, algo), grp in results_df.groupby(["dataset", "algorithm"]):
        ndcg10 = grp[(grp["metric"] == "NDCG") & (grp["k"].astype(str) == "10")]["value"].astype(float)
        if len(ndcg10) == 0:
            ndcg10 = grp[(grp["metric"] == "NDCG") & (grp["k"] == 10)]["value"].astype(float)
        analysis_rows.append({
            "dataset": dataset_name,
            "algorithm": algo,
            "n_runs": grp["seed"].nunique(),
            "ndcg@10_mean": float(ndcg10.mean()) if len(ndcg10) else float("nan"),
            "ndcg@10_std": float(ndcg10.std(ddof=1)) if len(ndcg10) > 1 else 0.0,
            "ndcg@10_cv": float(ndcg10.std(ddof=1) / ndcg10.mean()) if len(ndcg10) > 1 and ndcg10.mean() != 0 else 0.0,
        })
    ...
    meta = {
        "datasets": datasets,
        "seeds": seeds,
        "split": {"type": "RandomHoldout", "validation_size": 0.0, "test_size": 0.2},
        "preprocessing": {
            "core_pruning": 5,
