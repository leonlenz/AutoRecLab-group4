...
def run_single_seed(seed, dataset_name):
    ...
    # Valid user-based holdout. OmniRec's documented API requires positive validation_size and test_size.
    # To approximate the requested 80/20 holdout while keeping the API valid, we use a small validation split.
    ds = UserHoldout(validation_size=0.05, test_size=0.20).process(ds)
...
def main():
    ...
    meta = {
        "datasets": datasets,
        "seeds": seeds,
        "split": {"type": "UserHoldout", "validation_size": 0.05, "test_size": 0.20},
        "preprocessing": {
            "core_pruning": 5,
