...
def main():
    ...
    raw_rows = []
    protocol_rows = []
    per_dataset_seed_summaries = defaultdict(list)
    ...
    for ds_name, ds_id in dataset_map.items():
        ...
        for seed in seeds:
            ...
            # summarize within seed for later analysis
            seed_summary = (
                result_df.groupby(['dataset', 'algorithm', 'name', 'k'], as_index=False)['value']
                .mean()
            )
            per_dataset_seed_summaries[ds_name].append(seed_summary)
        ...
        for metric in ['NDCG', 'Precision']:
            for k in ks:
                ...
                analysis_lines.append(
                    f'{ds_name} | {metric}@{k}: best={best_algo}, mean={best_mean:.4f}, sd={best_std:.4f}, '
                    f'cv={cv:.3f}, spread={spread:.4f}'
                )

    # additional short statistical note about seed sensitivity
    variability_rows = []
