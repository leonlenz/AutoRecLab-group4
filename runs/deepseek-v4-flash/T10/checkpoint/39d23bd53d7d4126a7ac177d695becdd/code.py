...
for seed_idx, seed in enumerate(SEEDS):
    ...
    for ds_name, ds_config in DATASET_CONFIGS.items():
        ...
        # Add MakeImplicit if needed (MovieLens100K and Amazon have explicit ratings > 3)
        ...
        # User-based 80/20 holdout split (test=0.2, no validation set)
        steps.append(UserHoldout(validation_size=0, test_size=0.2))
        ...
        # Print dataset stats
        n_train = len(dataset._data.train)
        n_test = len(dataset._data.test)
        n_users = dataset._data.train['user'].nunique()
        n_items = dataset._data.train['item'].nunique()
    ...
    for dataset_id, df in results_dict.items():
        ...
        # The dataset_id contains the dataset name as a prefix
        matched_name = None
    ...
    print(f"  -> Extracted {len(df)} metric rows for seed {seed}")
