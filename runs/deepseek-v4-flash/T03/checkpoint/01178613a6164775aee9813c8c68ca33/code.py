...
def preprocess_dataset(dataset_name: str, make_implicit: bool = True):
    ...
    steps = []
    if make_implicit:
        # Use threshold 4 to keep only ratings > 3 (i.e., ratings 4 and 5)
    ...
    # Extract the dataframe - OmniRec DataFrame has columns: user, item, rating, timestamp
...
def evaluate_run(recs, truths):
    ...
    summary_metrics() to compute aggregated metrics.
    ...
    # Use the stateful two-step API (add_collection_measurements + summary_metrics)
    # instead of measure_run, to avoid type checker resolution issues.
    ...
    for ds_label, ds_key, make_impl in datasets_config:
        ...
        for algo_name in algorithms:
            ...
            for seed_idx, seed in enumerate(seeds):
                ...
                # OmniRec uses columns: user, item, rating, timestamp
                # from_interactions_df auto-detects these column names
                ...
                # Use split.test.keys() which returns UserIDKey objects
                ...
                ndcg10_key = f"NDCG.{10}.mean"
                precision10_key = f"Precision.{10}.mean"
                ndcg_val = summary.get(ndcg10_key, 'N/A')
                prec_val = summary.get(precision10_key, 'N/A')
                print(f"NDCG@10={ndcg_val:.4f}, Precision@10={prec_val:.4f}")
