...
for dataset_name, ds_config in DATASET_CONFIGS.items():
    ...
    for seed_idx, seed in enumerate(SEEDS):
        ...
        # UserHoldout: 80% train, 0% validation, 20% test
        # We use validation_size=0.0 since the task says 80/20 holdout
        pipe_steps.append(UserHoldout(validation_size=0.0, test_size=0.2))

        pipeline = Pipe(*pipe_steps)

        print(f"    Preprocessing (MakeImplicit={make_implicit}, CorePruning=5, UserHoldout=80/20)...")
        processed_dataset = pipeline.process(dataset)

        # Verify we got a SplitData
