...
def main():
    ...
    for dataset_idx, (raw_ds, ds_name, needs_implicit) in enumerate([
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            # UserHoldout: 80% train, 20% test (no validation set)
            steps.append(UserHoldout(validation_size=0.0, test_size=0.2))

            pipeline = Pipe(*steps)
            processed = pipeline.process(raw_ds)

            # Tag the dataset via its meta name (it already carries provenance)
            all_preprocessed_datasets.append(processed)

            print(f"  [{ds_name}] seed={seed}: "
                  f"train={len(processed._data.get('train'))}, "
                  f"test={len(processed._data.get('test'))}")
    ...
    # We'll build a unified DataFrame with columns:
    # dataset_name, seed, algorithm, metric, k, value
    # Determine which seed each result entry corresponds to based on ordering.
    # Results dict has 15 keys (3 datasets x 5 seeds) in the order we passed
    # datasets to run_omnirec.
    all_rows = []
    ds_name_map = ["MovieLens100K", "Amazon2014VideoGames", "HetrecLastFM"]

    for idx, (dataset_id, df) in enumerate(results_dict.items()):
        ds_base_idx = idx // len(seeds)  # 0,1,2 for the 3 datasets
        seed_idx = idx % len(seeds)      # 0..4 for the 5 seeds
        ds_name = ds_name_map[ds_base_idx]
