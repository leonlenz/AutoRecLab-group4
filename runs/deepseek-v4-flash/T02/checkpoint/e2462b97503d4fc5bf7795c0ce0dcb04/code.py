...
for ds_name, ds_cfg in DATASETS_CFG.items():
    ...
    loaded_datasets[ds_name] = ds_cfg
...
for ds_name, ds_cfg in loaded_datasets.items():
    print(f"\n{'─' * 70}")
    ...
    for seed_idx, seed in enumerate(SEEDS):
        ...
        # ── Step 1: Preprocess ──
        # Start from a fresh raw copy for each seed
        dataset = RecSysDataSet.use_dataloader(ds_cfg["dataset_enum"])
        ...
        # CRITICAL FIX: Use validation_size=0.001 instead of 0
        # UserHoldout always creates 3 splits (train/val/test).
        # validation_size=0 causes test_size=0/(1-0.2)=0.0 in train_test_split, which fails.
        # Using 0.001 gives ~79.9%/0.1%/20% split - approximately 80/20.
        set_random_state(seed)
        pipe_steps.append(UserHoldout(validation_size=0.001, test_size=0.2))
        ...
        # chdir to working_dir so checkpoints go to ./working/checkpoints
        try:
            run_omnirec(
