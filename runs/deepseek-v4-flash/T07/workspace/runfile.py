...
def main():
    ...
    for seed_idx, seed in enumerate(SEEDS):
        ...
        for ds_name, ds_enum in DATASET_NAMES.items():
            ...
            if ds_name == "HetrecLastFM":
                ...
                pipeline = Pipe(
                    CorePruning(5),
                    # Use a small positive validation_size to avoid sklearn crash
                    # (validation_size=0.0 leads to test_size=0.0 in internal second split)
                    UserHoldout(validation_size=0.05, test_size=0.2),
                )
            else:
        ...
        print(f"  Running experiment plan (3 algorithms x {len(processed_datasets)} datasets)...")
        sys.stdout.flush()
    ...
    for ds_name_short in ["MovieLens100K", "Amazon2014VideoGames", "HetrecLastFM"]:
        print(f"\n{'─' * 80}")
        print(f"Dataset: {ds_name_short}")
        print(f"{'─' * 80}")

        ds_stats = stats[stats["ds_short"] == ds_name_short]
