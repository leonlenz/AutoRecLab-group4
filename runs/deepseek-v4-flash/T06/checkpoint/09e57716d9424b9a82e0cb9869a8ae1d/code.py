...
def main():
    ...
    #
    # IMPORTANT: UserHoldout requires validation_size > 0 because it internally
    # computes test_size = valid_size / (1 - test_size) for the second split.
    # Using validation_size=0.0 passes 0.0 to sklearn's train_test_split which
    # rejects it. So we use a tiny positive validation_size (0.001) to get
    # effectively ~80/20 train/test with negligible validation.
    # -----------------------------------------------------------------------
    raw_datasets = {}
    for ds_name, ds_enum in DATASET_NAMES.items():
        ...
        print(f"  After preprocessing: {ds.num_interactions()} interactions")
    ...
    for seed_idx, seed in enumerate(RANDOM_SEEDS):
        ...
        for ds_name, raw_ds in raw_datasets.items():
            ...
            # Apply user-based holdout split: ~80% train, ~0.1% val, ~20% test
            # Uses tiny validation_size to avoid sklearn error with 0.0
            splitter = UserHoldout(validation_size=0.001, test_size=0.2)
            split_ds = splitter.process(raw_ds)

            # Print basic stats using the public SplitData API
            train_df = split_ds._data.train
            val_df = split_ds._data.val
