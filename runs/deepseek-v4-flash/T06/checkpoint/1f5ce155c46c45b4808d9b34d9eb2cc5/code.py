...
ALGORITHMS = {
    ...
    LensKit.ImplicitMFScorer: {"features": 50, "epochs": 100},
}
...
for dataset_name, ds_config in DATASET_CONFIGS.items():
    ...
    for seed_idx, seed in enumerate(SEEDS):
        ...
        dataset = RecSysDataSet.use_dataloader(str(ds_config["enum"]))
        ...
        # Apply user-based 80/20 holdout split (validation_size=0.0, test_size=0.2)
        preprocess_steps.append(UserHoldout(validation_size=0.0, test_size=0.2))
...
if all_results:
    ...
    for (ds, algo, metric, k), group in grouped:
        ...
        if len(values) > 0:
            ...
            stats_rows.append({
                "Dataset": ds,
                "Algorithm": algo,
                "Metric": f"{metric}@{k}" if k is not None else metric,
    ...
    # Pivot table: CV across algorithms and datasets for NDCG@10
