...
try:
    ...
    raise ImportError(
        "This OmniRec installation does not expose omnirec.metrics.ranking.Precision. "
        "The requested experiment requires Precision@k, so please use an OmniRec version that provides it."
    ) from e
...
DATASET_SPECS = [
    ...
    ("HetrecLastFM", DataSet.HetrecLastFM, True),
]
...
def build_base_dataset(dataset_enum, convert_to_implicit):
    ...
    if convert_to_implicit:
        if dataset_enum in {DataSet.MovieLens100K, DataSet.Amazon2014VideoGames}:
...
def collect_metric_results(evaluator, dataset_name, seed):
    ...
    return out.sort_values(["algorithm", "name", "k"]).reset_index(drop=True)
...
def run_single_experiment(dataset_name, split_dataset, working_dir, seed):
    ...
    try:
        ...
        result_df = collect_metric_results(evaluator, dataset_name, seed)
    finally:
        os.chdir(old_cwd)
        ...
        return pd.DataFrame(
            {
                ...
                "k": pd.Series(dtype="int64"),
                "algo_a": pd.Series(dtype="object"),
                "algo_b": pd.Series(dtype="object"),
                "n_seeds": pd.Series(dtype="int64"),
                "mean_diff": pd.Series(dtype="float64"),
...
def main():
    ...
    print(f"Ks: {KS}")
    print(
        f"Using OmniRec UserHoldout(validation_size={VALIDATION_SIZE}, test_size={TEST_SIZE}) and reporting test fold only"
    )
    ...
    for dataset_name, dataset_enum, convert_to_implicit in DATASET_SPECS:
        ...
        print(f"Interactions after preprocessing: {base_dataset.num_interactions()}")

        for seed in SEEDS:
            print_section(f"Running dataset={dataset_name}, seed={seed}")
            split_dataset = build_seed_split(base_dataset, seed)
            split_counts = split_dataset.num_interactions()
            ...
            print(
                result_df[["dataset", "seed", "algorithm", "name", "k", "value"]]
                .sort_values(["algorithm", "name", "k"])
