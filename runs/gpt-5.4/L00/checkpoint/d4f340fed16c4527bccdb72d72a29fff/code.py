...
def get_test_truth_from_split(split_dataset):
    split_data = getattr(split_dataset, "_data", None)
    if split_data is None or not hasattr(split_data, "get"):
        raise TypeError("Expected RecSysDataSet containing SplitData with get(split) API")
    test_df = split_data.get("test").copy()
...
def main():
    ...
    print(f"Seeds: {SEEDS}")
    print(f"Ks: {KS}")
    print(f"Using OmniRec UserHoldout(validation_size={VALIDATION_SIZE}, test_size={TEST_SIZE})")
    ...
    for dataset_name, dataset_enum, convert_to_implicit in DATASET_SPECS:
        ...
        for seed in SEEDS:
            ...
            test_truth = get_test_truth_from_split(split_dataset)
            print(f"Unique test user-item pairs: {len(test_truth)}")
